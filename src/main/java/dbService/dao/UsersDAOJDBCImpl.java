...
public class UsersDAOJDBCImpl implements UsersDAO {
    ...
    public void createTable() throws DBException {
        try {
            executor.execUpdate("create table if not exists users (id bigint auto_increment,  login varchar(256) UNIQUE, password varchar(256), name varchar(256), primary key (id))");
    ...
    public void addUser(User user) throws DBException {
        try {
            executor.execUpdate("insert into users (login,password,name) values ('" + user.getLogin() + "','"
                    + user.getPassword() + "','"
                    + user.getName() + "')");
        } catch (SQLException e) {
            throw new DBException(e);
    ...
    public void upDateUser(User user) throws DBException {
        try {
            executor.execUpdate("UPDATE users Set password = '" + user.getPassword() + "', name ='" + user.getName() + "' where id = '" + user.getId() + "'");
        } catch (SQLException e) {
            throw new DBException(e);
        }
    }
